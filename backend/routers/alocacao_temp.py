...
@router.post("/processar-semestre")
def processar_alocacao_semestre(
    mes_inicio: Optional[int] = Body(None, embed=True),
    ano_inicio: Optional[int] = Body(None, embed=True),
    duracao_meses: int = Body(5, embed=True),
    db: Session = Depends(get_db)
):
    """
    Processa a alocação automática de docentes para um semestre completo (5 meses).
    
    Quando há conflitos de horários (múltiplos docentes disponíveis no mesmo dia/hora),
    o sistema implementa um esquema de recorrência alternada:
    - Semana 1: Professor A
    - Semana 2: Professor B  
    - Semana 3: Professor A
    - E assim por diante...
    
    Args:
        mes_inicio: Mês de início (1-12), padrão: mês atual
        ano_inicio: Ano de início, padrão: ano atual
        duracao_meses: Duração do semestre em meses, padrão: 5
        
    Returns:
        Resultado com eventos do calendário e estatísticas da alocação
    """
    try:
        resultado = processar_alocacao_semestre_completo(
            db=db,
        ...
        return {
            "success": True,
            "resultado": resultado,
            "mensagem": f"Alocação semestral processada! {resultado['periodo']['total_eventos']} eventos gerados para {resultado['periodo']['total_semanas']} semanas."
        }
        
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Erro ao processar alocação semestral: {str(e)}")
